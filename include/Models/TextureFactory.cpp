...
namespace bogong {
	...
	std::shared_ptr<bogong::Texture> TextureManager::make_texture(std::string path)
	{
		...
		else
		{
			...
			int height = 0;
			int chnls = 0;
			auto back = path.find_last_of('.');
			std::string ext(path.begin()+back+1, path.end());

			if (ext == "dds"|ext=="ktx") {
				GLuint val = create_texture(path.c_str());
			...
			if (data == 0) {
				std::cout << "Error, " << path << " texture file could not be opened.\n";
				std::system("pause");
			}
